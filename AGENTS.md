...
| Demo readiness | Ready (mock) | Runs end-to-end in mock mode; needs a Supabase project + migrations for storage/security. |
| Security | Hardened | Private manage tokens, RLS/anon revoke migrations, redacted logs, no-store API responses, basic security headers, rate limits. |
...
- `supabase/migrations/001_initial.sql`, `002_security_hardening.sql`: present (raffles, entries, gifts, manage-token hash, RLS/privilege hardening)
...
- `/api/health`: live, returns `{ ok }` in production; optional `checks` only when `HEALTH_DETAILS=true` or non-production
...
| T08 | Codex | Security and privacy hardening | done | Private manage-token auth, RLS/privilege migration, redacted logs, headers, rate limits, bounded inputs. |
...
- [~] `/api/health` returns `ok: true` — always a structured 200; `ok: true` requires Supabase + Bitrefill/mock configured. OpenAI is optional because deterministic selection is supported.

### Security And Privacy

- [x] Private manage links include a random token; token hash only is stored in Supabase
- [x] Draw endpoint requires both creator email and private manage token
- [x] Supabase migrations enable RLS and revoke anon/authenticated table access
- [x] Gift codes are never returned by public raffle APIs
- [x] Local email fallback logs redact gift codes, email addresses, and personal descriptions by default
- [x] API JSON responses use `Cache-Control: no-store`, `Referrer-Policy: no-referrer`, and `X-Content-Type-Options: nosniff`
- [x] Global browser security headers configured in `next.config.mjs`
- [x] Public mutation endpoints have basic in-memory rate limits and bounded input lengths
...
| 2026-06-17 | Creator auth requires private manage token + email | Email-only auth was too weak for purchase-triggering actions |
| 2026-06-17 | Store only manage-token hashes | Avoid leaking private manage links from database reads |
| 2026-06-17 | Redact gift codes and personal details from demo logs by default | Prevent accidental leakage in screenshots, recordings, or shared logs |
| 2026-06-17 | Enable Supabase RLS and revoke anon/authenticated table access | All data access should go through server API routes using service role |
...
| I07 | low | Creator auth was email-match only | resolved | Draw now requires private manage token + creator email. |
...
| I09 | medium | Existing Supabase projects that already ran the original migration need `supabase/migrations/002_security_hardening.sql` applied | open | Human |
...
provide a **Supabase project** (URL + anon + service role) and apply both
`supabase/migrations/001_initial.sql` and `supabase/migrations/002_security_hardening.sql`. For the demo set `BITREFILL_MOCK=true`
...
/ redacted console-logged email metadata).

IMPORTANT GAP FOR SUBMISSION: the real (non-mock) autonomous Bitrefill purchase
path is implemented, including per-order delivery polling and `redemption_info`
handling, but still needs live verification with real credentials/test credits
(tracked as T05/I05).
...
2. Apply the migrations in order: run `supabase/migrations/001_initial.sql`, then `supabase/migrations/002_security_hardening.sql`, in the Supabase SQL editor.
